...
function RegisterPage() {
  ...
  const inputClass =
    "w-full rounded-full bg-white px-5 py-3.5 text-[var(--ink)] placeholder:text-[color-mix(in_oklab,var(--ink)_35%,transparent)] outline-none transition focus:ring-2 focus:ring-[var(--ink)] disabled:opacity-60";
        ...
        <div className="relative w-full max-w-md mx-auto lg:max-w-lg flex-1 flex flex-col justify-center min-h-0">
          ...
          <form onSubmit={onSubmit} noValidate className="mt-8 lg:mt-10 space-y-5 [@media(max-height:700px)]:mt-6 [@media(max-height:700px)]:space-y-3">
            ...
            <button
              type="submit"
              disabled={submitting || success}
              className="group w-full rounded-full px-5 py-3.5 font-display uppercase tracking-wider text-sm transition hover:opacity-90 disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2"
              style={{ background: "var(--ink)", color: "var(--cream)" }}
            >
