...
function RegisterPage() {
  ...
  return (
    <div
      className="h-screen w-full grid lg:grid-cols-2 overflow-hidden animate-in fade-in duration-500"
    ...
    >
      ...
      >
        ...
        <div className="relative">
          <div className="eyebrow mb-5 opacity-75">Restricted access</div>
          <h1 className="display-lg leading-[0.95]">
            Request
            <br />
            <span className="hl">clinical</span>
            <br />
            access.
          </h1>
          <p className="mt-6 max-w-sm text-sm leading-relaxed opacity-80">
            Only emails on the approved partner-domain allowlist will be granted
            access. Requests are reviewed by the GeneScope team, and you'll
            receive a confirmation once your workspace is provisioned.
          </p>
          <ul className="mt-6 max-w-sm space-y-2 text-sm opacity-75">
            <li>· Use your institutional email.</li>
            <li>· Password must be at least 8 characters.</li>
            <li>· Access is logged under RA 10173.</li>
          </ul>
        </div>
      ...
      <section className="relative flex flex-col justify-between h-full p-6 sm:p-10 lg:p-12 xl:p-14 overflow-hidden">
        ...
        <div className="relative w-full max-w-md mx-auto lg:max-w-lg">
          ...
          <div className="mt-5 min-h-[58px]">
            {error && (
              <div role="alert" className="flex items-start gap-2 rounded-xl px-3 py-2.5 text-sm animate-in fade-in slide-in-from-top-1 duration-200" style={{ background: "color-mix(in oklab, var(--destructive) 12%, transparent)", color: "var(--destructive)", border: "1px solid color-mix(in oklab, var(--destructive) 35%, transparent)" }}>
                <AlertCircle className="h-4 w-4 mt-0.5 shrink-0" />
                <span>{error}</span>
              </div>
            )}
            {success && (
              <div role="status" className="flex items-start gap-2 rounded-xl px-3 py-2.5 text-sm animate-in fade-in slide-in-from-top-1 duration-200" style={{ background: "color-mix(in oklab, var(--teal) 15%, transparent)", color: "var(--teal-deep)", border: "1px solid color-mix(in oklab, var(--teal) 35%, transparent)" }}>
