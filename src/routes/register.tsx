...
function RegisterPage() {
  ...
  return (
    ...
    >
      ...
      {/* RIGHT — form */}
      <section className="relative flex flex-col h-full p-6 sm:p-10 lg:p-12 xl:p-14 overflow-hidden [@media(max-height:700px)]:p-4 sm:[@media(max-height:700px)]:px-6 sm:[@media(max-height:700px)]:py-4 lg:[@media(max-height:700px)]:px-8 lg:[@media(max-height:700px)]:py-4">
        <img
        ...
        <div className="relative w-full max-w-md mx-auto lg:max-w-lg flex-1 flex flex-col justify-center min-h-0">
          ...
          <form onSubmit={onSubmit} noValidate className="mt-8 lg:mt-10 space-y-5 [@media(max-height:700px)]:mt-5 [@media(max-height:700px)]:space-y-2">
            ...
            <button
              type="submit"
              disabled={submitting || success}
              className="group w-full rounded-full px-5 py-3.5 font-display uppercase tracking-wider text-sm transition hover:opacity-90 disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2 [@media(max-height:700px)]:py-2"
              style={{ background: "var(--ink)", color: "var(--cream)" }}
            >
